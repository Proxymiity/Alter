...
async def on_command_error(ctx, error):
    if isinstance(error, commands.NoPrivateMessage):
        ...
        await ctx.send(loc.load(dl, mn, "err_pm_only"))
    elif isinstance(error, commands.DisabledCommand):
        await ctx.send(loc.load(dl, mn, "err_disabled"))
    elif isinstance(error, commands.CommandOnCooldown):
        await ctx.send(loc.load(dl, mn, "err_cd").format(error.retry_after))
        ...
        if isinstance(error.original, discord.Forbidden):
            await ctx.send(loc.load(dl, mn, "err_missing_perm"))
        elif "Missing Permission" in "{}".format(error):
            await ctx.send(loc.load(dl, mn, "err_missing_perm"))
        elif "FORBIDDEN" in "{}".format(error):
            await ctx.send(loc.load(dl, mn, "err_missing_perm"))
        else:
            await ctx.send(loc.load(dl, mn, "err_exec").format(error))
        ...
        await ctx.send(loc.load(dl, mn, "err_uncaught"))
